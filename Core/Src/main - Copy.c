...
uint8_t cmd_MQTT[15] = {'+', 'C', 'M', 'Q', 'T', 'T', 'R', 'X', 'P', 'A', 'Y', 'L', 'O', 'A', 'D', ':'};
...
DELAY_SEND_DATA:
	...
	sprintf((char *)message_gui_server, "%d.%d %d.%d %d %d %d %d %d %d %d %d %d \n\n\n\n\n\n\n\n", data_dht11[2], data_dht11[3], data_dht11[0], data_dht11[1], adc_quang_tro, adc_do_am_dat, a, b,
	    c, d, check_led, check_quat, check_bom);
	//////////////////////////////////////////////////////////////////////
	HAL_UART_Transmit(&huart1, message_gui_server, strlen((const char *)message_gui_server), 2000);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART0:
	...
	HAL_UART_Transmit(&huart1, data_SIM0, strlen((const char *)data_SIM0), 100);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART1:
	...
	HAL_UART_Transmit(&huart1, data_SIM1, strlen((const char *)data_SIM1), 100);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART2:
	...
	HAL_UART_Transmit(&huart1, data_SIM2, strlen((const char *)data_SIM2), 100);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART3:
	...
	HAL_UART_Transmit(&huart1, data_SIM3, strlen((const char *)data_SIM3), 100);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART5:
	...
	HAL_UART_Transmit(&huart1, data_SIM10, (uint16_t)strlen((const char *)data_SIM10), 100);
	HAL_Delay(1000);
BUFFER5:
	err = check_buffer_RX();
	if (err) {
	...
	HAL_UART_Transmit(&huart1, data_SIM11, (uint16_t)strlen((const char *)data_SIM11), 100);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
static void MX_GPIO_Init(void)
{
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0 | GPIO_PIN_10 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_8 | GPIO_PIN_9, GPIO_PIN_RESET);
	...
	/*Configure GPIO pins : PB0 PB10 PB12 PB13
	                         PB8 PB9 */
	GPIO_InitStruct.Pin = GPIO_PIN_0 | GPIO_PIN_10 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_8 | GPIO_PIN_9;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if (htim->Instance == TIM4) {
		...
		if (!strcmp(data_RX_0, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_1, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_2, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_3, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_4, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_5, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_6, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_7, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_8, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_9, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_10, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_11, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		if (!strcmp(data_RX_12, cmd_OK)) {
			flag_pass = 1;
			flag_nhap_topic = 1;
		}
		...
		if (!err) {
			...
			__HAL_TIM_SET_COUNTER(&htim1, 29000);
			data_avail_mqtt = 1;
			count_test_re++;
		}
