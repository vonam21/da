...
uint8_t data_SIM7[40] = "AT+CMQTTPAYLOAD=0,50\r\n";
...
DELAY_SEND_DATA:
	...
	sprintf((char *)message_gui_server,
	    ...
	    check_bom);
	//////////////////////////////////////////////////////////////////////
	HAL_UART_Transmit(&huart1, message_gui_server, strlen(message_gui_server), 5000);
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
...
UART4:
	...
	HAL_UART_Transmit(&huart1, data_SIM4, strlen((const char *)data_SIM4), 1000);  // 60bytes
	while (flag_pass == 0) {
		HAL_Delay(500);
		count_again_trans++;
		if (flag_pass == 1) {
